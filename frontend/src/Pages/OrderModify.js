...
const OrderModify = ({orderUpdate, order}) => {
  ...
  const discount=40.0;
    ...
    const submitAddOrder= async (event) =>{
        ...
        dispatch(
          ...
          try {
            const response = await fetch('http://localhost:3001/storeOrder', {
              ...
              body: JSON.stringify({
                ...
                zipCode,
                orderType,
                orderDetails: [{id:productName,quantity:productQuantity}],
                instorePickup,
          ...
          navigate('/orderadded');
            ...
            try {
              const response = await fetch(`http://localhost:3001/updateOrder/${orderId}`, {
                method: 'PUT',
                headers: {
                  'Content-Type': 'application/json',
                },
    ...
    const submitDeleteOrder = async(event)=>{
        ...
        try {
          const response = await fetch(`http://localhost:3001/deleteOrder/${orderId1}`, {
            method: 'DELETE',
          });
